...
const maxSize = [500000, 500000]; // Maximum size for the bounding box
...
export const MapBBox = function ({ onBboxChange, bbox, disabled, mapSize = defaultMapSize, setAreaBbox, setCoordinatesToDisplay, coordinatesToDisplay }: 
     ...
     */
    const setBboxDescription = useCallback((points?: Array<Array<number>> | null) => {
        const bboxCornerPoints = points?.length ? [...points[0], ...points[2]] : null;
        if (bboxCornerPoints && distanceScales?.unitsPerDegree && distanceScales?.metersPerUnit) {
    ...
    return (
        <div style={{ width: `${mapSize[0]}px`, height: `${mapSize[1]}px`, position: 'relative' }}>
            <BoundingBox
