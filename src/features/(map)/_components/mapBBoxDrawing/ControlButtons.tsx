		...
		<>
			{CustomButtonsComponent && isValidElement(CustomButtonsComponent) ? (
				cloneElement(CustomButtonsComponent, {
					//@ts-expect-error cant find the type for this
					handleEditToggle,
					clearPoints,
					isActive,
				...
				<Stack className="mapBBoxDrawing-buttons" style={customStyles}>
					<Tooltip label={getTooltipLabel()}>
						<ActionIcon className="mapBBoxDrawing-addBtn" variant="default" onClick={handleEditToggle}>
							{getEditIcon()}
						</ActionIcon>
					</Tooltip>
					<Tooltip label={'Clear selection'}>
						<ActionIcon className="mapBBoxDrawing-addBtn" variant="default" onClick={clearPoints}>
							<IconTrash className="mapBBoxDrawing-buttons-icons" />
						</ActionIcon>
					</Tooltip>
				</Stack>
			)}
