...
export const editLayerBorder = (
	...
	originLinePointCoordinates: Array<Array<number>>,
	...
	setOriginalBboxBorderCoordinates: (current: Array<Array<number>>) => void
) => {
	...
	if (previousPoint && currentPoint && coordinates?.length && activeBboxPoints && originLinePointCoordinates?.length > 0) {
		const diffLat = currentPoint[0] - previousPoint[0]; // Change in latitude
		const diffLong = currentPoint[1] - previousPoint[1]; // Change in longitude
		...
		const diff = sharedCoordinatePart - oppositeCoordinatePart;

		// Check if the bbox can be made smaller
		const bboxCanBeSmaller = diff > 0 ? newSharedCoordinatePart > oppositeCoordinatePart + minBboxArea : newSharedCoordinatePart < oppositeCoordinatePart - minBboxArea;

		// Update the bounding box points based on the drag
		const newPoints = activeBboxPoints.map((point: Array<number>) => {
			...
			if (originPoint && bboxCanBeSmaller) {
				...
				if (originLinePointCoordinates[0][0] === originLinePointCoordinates[1][0]) {
					let newLat = originPoint[0] + diffLat; // Calculate new latitude
					// Check if the new latitude is within the available area
					if (updatedAvailableArea && (newLat < updatedAvailableArea[0][0] || newLat > updatedAvailableArea[2][0])) {
						newLat = findClosest(updatedAvailableAreaLat, newLat);
					}
					const newPoint = [newLat, originPoint[1]];
					// @ts-ignore
					setOriginalBboxBorderCoordinates((current: Array<Array<number>>) => [...current, newPoint]); // Update original bbox border coordinates
					return(newPoint);
					// Adjust longitude if the line is horizontal
				} else if (originLinePointCoordinates[0][1] === originLinePointCoordinates[1][1]) {
					let newLong = originPoint[1] + diffLong; // Calculate new longitude
					// Check if the new longitude is within the available area
					if (updatedAvailableArea && (newLong < updatedAvailableArea[0][1] || newLong > updatedAvailableArea[2][1])) {
